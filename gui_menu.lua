...
-- HÀM TẢI CONFIG (Đọc từ file khi vào game)
...
local function makeDraggable(uiInstance)
    ...
    uiInstance.InputBegan:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and not dragging then
            dragging = true
            currentTouchObject = input
            dragStart = input.Position
            startPos = uiInstance.Position
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false; currentTouchObject = nil
                end
            end)
        end
    end)
    ...
    uiInstance.InputChanged:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and dragging then
            if input == currentTouchObject or input.UserInputType == Enum.UserInputType.MouseMovement then
                dragInput = input
            end
        end
    end)
...
moveButton.Size = UDim2.new(0, 72, 0, 25)
moveButton.Position = UDim2.new(0, 83, 0, 135)
...
increaseSizeButton.Position = UDim2.new(0, 83, 0, 165)
...
tpSquare.Text = "TP"
tpSquare.TextColor3 = Color3.new(1, 1, 1)
tpSquare.Font = Enum.Font.SourceSansBold
tpSquare.TextSize = 16
tpSquare.Visible = tpaEnabled
tpSquare.Active = true
        ...
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if distance < shortestDistance then
                    shortestDistance = distance; closestPlayer = v
                end
            end
        end
    end
...
decreaseButton.MouseButton1Click:Connect(function() 
    if speedValue > 16 then 
        ...
        MySettings.speedValue = speedValue; SaveConfig()
    end 
end)
...
decreaseSizeButton.MouseButton1Click:Connect(function()
    if tpSizeValue > 30 then
        ...
        MySettings.tpSizeValue = tpSizeValue; SaveConfig()
    end
end)

increaseSizeButton.MouseButton1Click:Connect(function()
    if tpSizeValue < 150 then
        tpSizeValue = tpSizeValue + 10; tpSquare.Size = UDim2.new(0, tpSizeValue, 0, tpSizeValue)
        MySettings.tpSizeValue = tpSizeValue; SaveConfig()
    end
end)
...
task.spawn(function()
    while true do
        ...
        if safeEnabled and safePart and safePart.Parent then
            ...
            if hrp and hrp.Position.Y < (safePart.Position.Y - 10) then
                hrp.CFrame = CFrame.new(safePart.Position + Vector3.new(0, 3, 0))
            end
        end
    end
...
local function createESP(player)
    ...
    local function applyESP(character)
        ...
        nameLabel.BackgroundTransparency = 1; nameLabel.Text = player.Name; nameLabel.TextColor3 = Color3.new(1, 1, 1)
        ...
        connection = RunService.RenderStepped:Connect(function()
            ...
            if onScreen then
                ...
                tracerFrame.Size = UDim2.new(0, distance, 0, 1.5)
                tracerFrame.Position = UDim2.new(0, (startX + endX) / 2, 0, (startY + endY) / 2)
...
Players.PlayerRemoving:Connect(function(p)
    if espObjects[p] then
        ...
        if espObjects[p].Tracer then espObjects[p].Tracer:Destroy() end
        espObjects[p] = nil
    end
end)

tpButton.MouseButton1Click:Connect(function()
    tpNearestEnabled = not tpNearestEnabled
    tpButton.Text = tpNearestEnabled and "TP: ON" or "TP: OFF"
    tpButton.BackgroundColor3 = tpNearestEnabled and Color3.fromRGB(0, 200, 100) or Color3.fromRGB(255, 100, 0)
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.5); checkSafePlatform(); updateESPStatus()
end)

minimizeButton.MouseButton1Click:Connect(function()
    frame.Visible = false; openButton.Position = frame.Position; openButton.Visible = true
end)

openButton.MouseButton1Click:Connect(function()
    openButton.Visible = false; frame.Position = openButton.Position; frame.Visible = true
end)
