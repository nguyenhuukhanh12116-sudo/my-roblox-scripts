...
local addFloatBtn = createBtn("addFBtn", "Thêm Nút", 75, 0, 0, Color3.fromRGB(0, 150, 255), floatTpFrame)
local delFloatBtn = createBtn("delFBtn", "Xóa Nút", 75, 85, 0, Color3.fromRGB(200, 50, 50), floatTpFrame)
local lockDragBtn = createBtn("lockDBtn", "Khóa Di Chuyển", 160, 0, 35, C_ON_GRN, floatTpFrame)
local decFSizeBtn = createBtn("decFSBtn", "Kích Cỡ -", 75, 0, 70, Color3.fromRGB(180, 100, 20), floatTpFrame)
local incFSizeBtn = createBtn("incFSBtn", "Kích Cỡ +", 75, 85, 70, Color3.fromRGB(180, 150, 20), floatTpFrame)

local isSafeOpen, isTpOpen, isFloatTpOpen = false, false, false
local function updateMainFrameSize()
    local baseHeight = 330
    local col2Height = 10 + 35 + (isSafeOpen and 70 or 0) + 35 + (isTpOpen and 105 or 0) + 35 + (isFloatTpOpen and 110 or 0)
...
lockDragBtn.MouseButton1Click:Connect(function()
    floatDraggable = not floatDraggable
    lockDragBtn.Text = floatDraggable and "Đang Mở Kéo" or "Đã Khóa Kéo"
    lockDragBtn.BackgroundColor3 = floatDraggable and C_ON_GRN or Color3.fromRGB(200, 50, 50)
end)
...
addFloatBtn.MouseButton1Click:Connect(function()
    ...
    f.InputBegan:Connect(function(input)
        if floatDraggable and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
...
fogBtn.MouseButton1Click:Connect(function()
    ...
    if noFogEnabled then Lighting.FogEnd = 100000; for _, v in pairs(Lighting:GetDescendants()) do if v:IsA("Atmosphere") then v:Destroy() end end end
end)
...
clickTpBtn.MouseButton1Click:Connect(function()
    ...
    if clickTpEnabled then
        if not clickTpTool then
            clickTpTool = Instance.new("Tool"); clickTpTool.Name = "Click To TP"; clickTpTool.RequiresHandle = false; clickTpTool.Parent = LocalPlayer.Backpack
...
local function checkSafePlatform()
    if safeEnabled then
        ...
        if hrp and (not safePart or not safePart.Parent) then
            safePart = Instance.new("Part"); safePart.Size = Vector3.new(20, 1, 20); safePart.Position = Vector3.new(hrp.Position.X, hrp.Position.Y + 300, hrp.Position.Z)
...
safeSquare.MouseButton1Click:Connect(function()
    safeEnabled = not safeEnabled; safeBtn.Text = safeEnabled and "Safe: ON" or "Safe: OFF"; safeBtn.BackgroundColor3 = safeEnabled and Color3.fromRGB(255, 150, 0) or C_OFF; safeSquare.Text = safeEnabled and "SF: ON" or "SAFE"; safeSquare.BackgroundColor3 = safeEnabled and C_ON_GRN or Color3.fromRGB(255, 180, 0); MySettings.safeEnabled = safeEnabled; SaveConfig(); checkSafePlatform()
    if safeEnabled then task.wait(0.05); if safePart and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(safePart.Position + Vector3.new(0, 3, 0)) end end
end)
...
local function triggerAttackBlink()
    if flingEnabled and not isAttacking then
        local myHrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart"); local target = getClosestPlayer()
        if myHrp and target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            if (myHrp.Position - target.Character.HumanoidRootPart.Position).Magnitude < 25 then
...
local function trackWeapon(character) if toolConnection then toolConnection:Disconnect() end; toolConnection = character.ChildAdded:Connect(function(child) if child:IsA("Tool") then child.Activated:Connect(triggerAttackBlink) end end) end
