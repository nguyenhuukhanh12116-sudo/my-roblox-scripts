local SETTINGS_FILE, Players, LocalPlayer = "JNHHGaming_Config.json", game:GetService("Players"), game:GetService("Players").LocalPlayer
local UserInputService, HttpService = game:GetService("UserInputService"), game:GetService("HttpService")
local MySettings = {speedValue = 50, infiniteJumpEnabled = false, safeEnabled = false, tpNearestEnabled = false, espEnabled = false, tpaEnabled = false, moveEnabled = false, showSafeSquare = true, chamsEnabled = false, hitboxEnabled = false, tpSizeValue = 50, safeSizeValue = 50, tpSquareX_Scale = 0.5, tpSquareX_Offset = -55, tpSquareY_Scale = 0.5, tpSquareY_Offset = -25, safeSquareX_Scale = 0.5, safeSquareX_Offset = 5, safeSquareY_Scale = 0.5, safeSquareY_Offset = -25}
...
pcall(function() if readfile and isfile and isfile(SETTINGS_FILE) then local decoded = HttpService:JSONDecode(readfile(SETTINGS_FILE)) if type(decoded) == "table" then for k, v in pairs(decoded) do MySettings[k] = v end end end end)
local speedValue, infiniteJumpEnabled, safeEnabled, tpNearestEnabled, espEnabled, tpaEnabled, moveEnabled, showSafeSquare, chamsEnabled, hitboxEnabled, tpSizeValue, safeSizeValue, safePart, espObjects, chamsObjects = MySettings.speedValue, MySettings.infiniteJumpEnabled, MySettings.safeEnabled, MySettings.tpNearestEnabled, MySettings.espEnabled, MySettings.tpaEnabled, MySettings.moveEnabled, MySettings.showSafeSquare, MySettings.chamsEnabled, MySettings.hitboxEnabled, MySettings.tpSizeValue, MySettings.safeSizeValue, nil, {}, {}
local gui = Instance.new("ScreenGui"); gui.Name = "JNHHGamingCompact"; gui.ResetOnSpawn = false; gui.Parent = LocalPlayer:WaitForChild("PlayerGui")
local frame = Instance.new("Frame"); frame.Name = "CompactFrame"; frame.Size = UDim2.new(0, 160, 0, 290); frame.Position = UDim2.new(0, 50, 0, 50); frame.BackgroundColor3 = Color3.fromRGB(20, 20, 20); frame.BackgroundTransparency = 0.3; frame.BorderSizePixel = 0; frame.Active = true; frame.Parent = gui; Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 6)
local openButton = Instance.new("TextButton"); openButton.Name = "OpenButton"; openButton.Size = UDim2.new(0, 80, 0, 30); openButton.Position = UDim2.new(0, 50, 0, 50); openButton.BackgroundColor3 = Color3.fromRGB(0, 120, 255); openButton.TextColor3 = Color3.new(1, 1, 1); openButton.Text = "Mở GUI"; openButton.Visible = false; openButton.Parent = gui; Instance.new("UICorner", openButton).CornerRadius = UDim.new(0, 4)
local function makeDraggable(uiInstance)
    ...
    uiInstance.InputBegan:Connect(function(input) if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and not dragging then dragging = true; currentTouchObject = input; dragStart = input.Position; startPos = uiInstance.Position; input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then dragging = false; currentTouchObject = nil end end) end end)
    uiInstance.InputChanged:Connect(function(input) if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and dragging then if input == currentTouchObject or input.UserInputType == Enum.UserInputType.MouseMovement then dragInput = input end end end)
    UserInputService.InputChanged:Connect(function(input) if input == dragInput and dragging then local delta = input.Position - dragStart; uiInstance.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y) end end)
end
makeDraggable(frame); makeDraggable(openButton)
local decreaseButton = Instance.new("TextButton"); decreaseButton.Size = UDim2.new(0, 30, 0, 25); decreaseButton.Position = UDim2.new(0, 5, 0, 10); decreaseButton.BackgroundColor3 = Color3.fromRGB(255, 50, 50); decreaseButton.Text = "-"; decreaseButton.TextColor3 = Color3.new(1, 1, 1); decreaseButton.Parent = frame; Instance.new("UICorner", decreaseButton).CornerRadius = UDim.new(0, 4)
local speedLabel = Instance.new("TextLabel"); speedLabel.Size = UDim2.new(0, 80, 0, 25); speedLabel.Position = UDim2.new(0, 40, 0, 10); speedLabel.BackgroundTransparency = 1; speedLabel.TextColor3 = Color3.new(1, 1, 1); speedLabel.Text = "Speed: " .. speedValue; speedLabel.Parent = frame
local increaseButton = Instance.new("TextButton"); increaseButton.Size = UDim2.new(0, 30, 0, 25); increaseButton.Position = UDim2.new(0, 125, 0, 10); increaseButton.BackgroundColor3 = Color3.fromRGB(50, 255, 50); increaseButton.Text = "+"; increaseButton.TextColor3 = Color3.new(1, 1, 1); increaseButton.Parent = frame; Instance.new("UICorner", increaseButton).CornerRadius = UDim.new(0, 4)
local function createBtn(text, pos, bg)
    local b = Instance.new("TextButton"); b.Size = UDim2.new(0, 72, 0, 25); b.Position = pos; b.BackgroundColor3 = bg; b.TextColor3 = Color3.new(1, 1, 1); b.Font = Enum.Font.SourceSansBold; b.TextSize = 12; b.Text = text; b.Parent = frame; Instance.new("UICorner", b).CornerRadius = UDim.new(0, 4)
    return b
end
...
local safeButton = createBtn(safeEnabled and "Safe: ON" or "Safe: OFF", UDim2.new(0, 83, 0, 45), safeEnabled and Color3.fromRGB(255, 200, 0) or Color3.fromRGB(255, 255, 0)); safeButton.TextColor3 = Color3.new(0, 0, 0)
...
local moveButton = Instance.new("TextButton"); moveButton.Size = UDim2.new(0, 150, 0, 25); moveButton.Position = UDim2.new(0, 5, 0, 165); moveButton.BackgroundColor3 = moveEnabled and Color3.fromRGB(0, 200, 100) or Color3.fromRGB(130, 130, 130); moveButton.TextColor3 = Color3.new(1, 1, 1); moveButton.Font = Enum.Font.SourceSansBold; moveButton.Text = moveEnabled and "Move: ON" or "Move: OFF"; moveButton.Parent = frame; Instance.new("UICorner", moveButton).CornerRadius = UDim.new(0, 4)
...
local minimizeButton = Instance.new("TextButton"); minimizeButton.Size = UDim2.new(0, 150, 0, 25); minimizeButton.Position = UDim2.new(0, 5, 0, 255); minimizeButton.BackgroundColor3 = Color3.fromRGB(200, 50, 50); minimizeButton.TextColor3 = Color3.new(1, 1, 1); minimizeButton.Text = "Thu nhỏ (-)"; minimizeButton.Parent = frame; Instance.new("UICorner", minimizeButton).CornerRadius = UDim.new(0, 4)
local tpSquare = Instance.new("TextButton"); tpSquare.Size = UDim2.new(0, tpSizeValue, 0, tpSizeValue); tpSquare.Position = UDim2.new(MySettings.tpSquareX_Scale, MySettings.tpSquareX_Offset, MySettings.tpSquareY_Scale, MySettings.tpSquareY_Offset); tpSquare.BackgroundColor3 = Color3.fromRGB(255, 0, 0); tpSquare.Text = "TP"; tpSquare.TextColor3 = Color3.new(1, 1, 1); tpSquare.Font = Enum.Font.SourceSansBold; tpSquare.TextSize = 16; tpSquare.Visible = tpaEnabled; tpSquare.Active = true; tpSquare.BorderSizePixel = 0; tpSquare.Parent = gui
local safeSquare = Instance.new("TextButton"); safeSquare.Size = UDim2.new(0, safeSizeValue, 0, safeSizeValue); safeSquare.Position = UDim2.new(MySettings.safeSquareX_Scale, MySettings.safeSquareX_Offset, MySettings.safeSquareY_Scale, MySettings.safeSquareY_Offset); safeSquare.BackgroundColor3 = Color3.fromRGB(255, 200, 0); safeSquare.Text = "SAFE"; safeSquare.TextColor3 = Color3.new(0, 0, 0); safeSquare.Font = Enum.Font.SourceSansBold; safeSquare.TextSize = 14; safeSquare.Visible = (safeEnabled and showSafeSquare); safeSquare.Active = true; safeSquare.BorderSizePixel = 0; safeSquare.Parent = gui
local function setupSquareDrag(targetUi, settingPrefix)
    ...
    targetUi.InputBegan:Connect(function(input) if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and moveEnabled and not dragging then dragging = true; touchObject = input; dragStart = input.Position; startPos = targetUi.Position; input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then dragging = false; touchObject = nil; MySettings[settingPrefix.."X_Scale"] = targetUi.Position.X.Scale; MySettings[settingPrefix.."X_Offset"] = targetUi.Position.X.Offset; MySettings[settingPrefix.."Y_Scale"] = targetUi.Position.Y.Scale; MySettings[settingPrefix.."Y_Offset"] = targetUi.Position.Y.Offset; SaveConfig() end end) end end)
    targetUi.InputChanged:Connect(function(input) if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and dragging and moveEnabled then if input == touchObject or input.UserInputType == Enum.UserInputType.MouseMovement then dragInput = input end end end)
    UserInputService.InputChanged:Connect(function(input) if input == dragInput and dragging and moveEnabled then local delta = input.Position - dragStart; targetUi.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y) end end)
end
...
local function createESP(player)
    ...
    local function applyESP(character)
        ...
        local nameLabel = Instance.new("TextLabel"); nameLabel.Size = UDim2.new(1, 0, 1, 0); nameLabel.BackgroundTransparency = 1; nameLabel.Text = player.Name; nameLabel.TextColor3 = Color3.fromRGB(255, 0, 0); nameLabel.TextStrokeTransparency = 0; nameLabel.TextStrokeColor3 = Color3.new(0, 0, 0); nameLabel.Font = Enum.Font.SourceSansBold; nameLabel.TextSize = 18; nameLabel.Parent = billboard; espObjects[player] = {Billboard = billboard}
    end
...
local function createChams(player)
    ...
    local function applyChams(character)
        ...
        local h = Instance.new("Highlight"); h.Name = "Chams_Highlight"; h.FillColor = Color3.fromRGB(255, 0, 0); h.OutlineColor = Color3.fromRGB(255, 255, 255); h.FillTransparency = 0.5; h.OutlineTransparency = 0; h.Adornee = character; h.Parent = character; chamsObjects[player] = h
    end
    if player.Character then applyChams(player.Character) end; player.CharacterAdded:Connect(applyChams)
end
        ...
        pcall(function()
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    ...
                    if hrp then
                        if hitboxEnabled then hrp.Size = Vector3.new(10, 10, 10); hrp.Transparency = 0.7; hrp.BrickColor = BrickColor.new("Really red"); hrp.Material = Enum.Material.ForceField; hrp.CanCollide = false
                        else hrp.Size = Vector3.new(2, 2, 1); hrp.Transparency = 1; hrp.CanCollide = false end
                    end
                end
            end
        end)
        if tpNearestEnabled then pcall(function() local targetPlayer = getClosestPlayer(); local myChar = LocalPlayer.Character; local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart"); if targetPlayer and myHrp then local targetHrp = targetPlayer.Character:FindFirstChild("HumanoidRootPart"); if targetHrp then myHrp.CFrame = targetHrp.CFrame end end end) end
    end
end)
task.spawn(function() while true do task.wait(0.5) if safeEnabled and safePart and safePart.Parent then pcall(function() local char = LocalPlayer.Character; local hrp = char and char:FindFirstChild("HumanoidRootPart") if hrp and hrp.Position.Y < (safePart.Position.Y - 10) then hrp.CFrame = CFrame.new(safePart.Position + Vector3.new(0, 3, 0)) end end) end end end)
